...
for file in "$SCRIPT_DIR"/e2e/*.sql; do
    ...
    if [ -f "$file" ] && [ "$filename" != "00_worker_setup.sql" ] && [ "$filename" != "21_audience_private_messages.sql" ]; then
        ...
        if [[ "$filename" == 17_private_message_deletion_w*.sql ]] || [[ "$filename" == 18_co_gm_management_w*.sql ]] || [[ "$filename" == 18_co_gm_action_results_w*.sql ]] || [[ "$filename" == 19_player_multiple_characters_w*.sql ]] || [[ "$filename" == 21_audience_private_messages_w*.sql ]] || [[ "$filename" == 23_private_message_editing_w*.sql ]]; then
            # Only process if it matches our worker index
            if [[ "$filename" == "17_private_message_deletion_w${WORKER_INDEX}.sql" ]] || [[ "$filename" == "18_co_gm_management_w${WORKER_INDEX}.sql" ]] || [[ "$filename" == "18_co_gm_action_results_w${WORKER_INDEX}.sql" ]] || [[ "$filename" == "19_player_multiple_characters_w${WORKER_INDEX}.sql" ]] || [[ "$filename" == "21_audience_private_messages_w${WORKER_INDEX}.sql" ]] || [[ "$filename" == "23_private_message_editing_w${WORKER_INDEX}.sql" ]]; then
